...
export const namify = (str: string) => {
    ...
    const name = str.normalize().split('')
...
export const toFixedNumberFormat = (num: number) => {
	return Number(num).toFixed(9).replace(/\.?0+$/,"");
}
...
export const removeExtraZerosNumber = (str: string) => {
	return str ? str.toString().replace(/^0+(?!\.)|(?:\.|(\..*?))0+$/gm, '$1') : 0;
}
...
export const formatMoneyToFixed = (params: formatMoneyToFixedType) => {
	try {
		...
		return negativeSign +
			...
			(decimalCount ? decimal + toFixed(`0.${amountSplit[1]}`, toFix ? toFix : decimalCount).slice(2) : "");
	} catch (e) {
		console.log(e)
		return 0;
	}
...
export const beforeUploadOnlyImagePool = (file: any) => {
	const isJpgOrPng = file.type === "image/jpeg" || file.type === "image/png" || file.type === "image/gif";
	if (!isJpgOrPng) {
		message.error("You can only upload JPG, JPEG, PNG, GIF file!").then();
		return false
	}
	const isAvailableSize = file.size / 1024 / 1024 < 10;
	if (!isAvailableSize) {
		message.error("File exceeded size limit. Please upload file below 10 MB.").then();
		return false
	}
	return isJpgOrPng && isAvailableSize;
}
